...
export default function Page() {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/trigger-disruption`, {
      method: "POST",
    });
    ...
    // The current backend deliberately does NOT execute the GPU solver.
    // Therefore, do not show fake solve time or recovery percentage.
    if (data.solver_executed === true) {
      setGpuCompleted(true);

      if (data.gpu_latency_seconds != null) {
        setGpuSolveTime(`${data.gpu_latency_seconds}s`);
      } else {
        setGpuSolveTime(null);
      }

      if (data.recovery_rate != null) {
        setRecoveryConfidence(data.recovery_rate);
      } else {
        setRecoveryConfidence("—");
      }

      // Only change flight statuses when the backend actually confirms
      // that the solver executed.
      setFlights((prev) =>
        prev.map((f) => ({
          ...f,
          status: "CLEARED",
          delay: "0 min",
          recoveryAction:
            data.recovery_action || "Recovery generated by optimization engine",
        }))
      );
    } else {
      // Backend is reachable, but GPU recovery has not actually executed.
      setGpuCompleted(false);
      setGpuSolveTime(null);
      setRecoveryConfidence("—");
    }
  } catch (error) {
    console.error("GPU recovery request failed:", error);

    // IMPORTANT:
    // Never simulate a successful GPU recovery when the backend is unavailable.
    setGpuCompleted(false);
    setGpuSolveTime(null);
    setRecoveryConfidence("—");
  } finally {
...
function BenchmarkPage({
  ...
  //
  // AYUSH LAPTOP:
  // NEXT_PUBLIC_API_URL=http://AYUSH_IP:8000
  // ---------------------------------------------------------------------------
  const API_BASE =
    process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000";
  ...
  //
  // This accepts the current ApexCUDA backend response and also handles
  // the real benchmark fields when Ayush's GPU benchmark endpoint returns them.
  // ---------------------------------------------------------------------------
  const normalizeBenchmarkResponse = (raw: any): BenchmarkData => {
    const gpuAvailable =
      raw?.gpu_available === true ||
      raw?.gpuAvailable === true ||
      raw?.gpu?.available === true;

    const benchmarkExecuted =
      raw?.benchmark_executed === true ||
      raw?.benchmarkExecuted === true ||
      raw?.executed === true;
    ...
    // If backend doesn't explicitly send speedup but sends both timings,
    // calculate it from the real measured values.
    if (speedup === null && cpuMs !== null && gpuMs !== null && gpuMs > 0) {
      speedup = cpuMs / gpuMs;
    }
    ...
    const throughput = toNumber(
      raw?.throughput ??
        raw?.gpu_throughput ??
        raw?.constraints_per_second ??
        raw?.gpu_constraints_per_second ??
        raw?.throughput_mps ??
        raw?.gpu?.throughput
    );
    ...
    const throughputUnit =
      raw?.throughput_unit ??
      raw?.throughputUnit ??
      raw?.gpu?.throughput_unit ??
      "constraints / sec";

    const maxError = toNumber(
      raw?.max_error ??
        raw?.maxError ??
        raw?.maximum_error ??
        raw?.error ??
    ...
    const match =
      typeof raw?.match === "boolean"
        ? raw.match
        : typeof raw?.results_match === "boolean"
        ? raw.results_match
        : typeof raw?.allclose === "boolean"
        ? raw.allclose
        : null;

    const gpuUtilization = toNumber(
      raw?.gpu_utilization ??
        raw?.gpuUtilization ??
        raw?.utilization ??
        raw?.gpu?.utilization
    ...
    const vramUsedGb = toNumber(
      raw?.vram_used_gb ??
        raw?.vramUsedGb ??
        raw?.memory_used_gb ??
        raw?.gpu?.vram_used_gb
    );

    const vramTotalGb = toNumber(
      raw?.vram_total_gb ??
        raw?.vramTotalGb ??
        raw?.memory_total_gb ??
        raw?.gpu?.vram_total_gb
    );

    const bandwidthGbps = toNumber(
      raw?.bandwidth_gbps ??
        raw?.bandwidthGbps ??
        raw?.interconnect_bandwidth_gbps ??
        raw?.gpu?.bandwidth_gbps
    );
  ...
  const handleRunBenchmark = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/benchmark-data`, {
        method: "GET",
        cache: "no-store",
      });
      ...
      const normalized = normalizeBenchmarkResponse(raw);

      setData(normalized);

      // Keep the parent state in sync with the existing dashboard.
      runAction();
    } catch (error) {
      ...
      setErrorMessage(
        "GPU backend unavailable — connect Ayush's GPU node to display benchmark results."
      );
    } finally {
  ...
  const benchmarkActive =
    data?.gpuAvailable === true && data?.benchmarkExecuted === true;

  const formatSpeedup = () => {
    if (!benchmarkActive || data?.speedup === null) return "—";
    return `${data.speedup.toFixed(2)}x`;
  };

  const formatGpuTime = () => {
    if (!benchmarkActive || data?.gpuMs === null) return "—";
    return `${data.gpuMs.toFixed(3)} ms`;
  };

  const formatCpuTime = () => {
    if (!benchmarkActive || data?.cpuMs === null) return "—";
    return `${data.cpuMs.toFixed(3)} ms`;
  };

  const formatThroughput = () => {
    if (!benchmarkActive || data?.throughput === null) return "—";

    // If backend already provides M/s, display it directly.
    if (
      data.throughputUnit?.toLowerCase().includes("m/s") ||
      data.throughputUnit?.toLowerCase().includes("million")
    ) {
      return `${data.throughput.toFixed(2)}M`;
    }

    // Otherwise assume constraints/sec and convert to millions/sec.
    if (data.throughput >= 1_000_000) {
      return `${(data.throughput / 1_000_000).toFixed(2)}M`;
    }

    return data.throughput.toFixed(2);
  };

  const formatUtilization = () => {
    if (!benchmarkActive || data?.gpuUtilization === null) return "—";
    return `${data.gpuUtilization.toFixed(0)}%`;
  };

  const formatVram = () => {
    if (!benchmarkActive || data?.vramUsedGb === null) return "—";

    if (data.vramTotalGb !== null) {
      return `${data.vramUsedGb.toFixed(1)} / ${data.vramTotalGb.toFixed(
        0
      )} GB`;
    }

    return `${data.vramUsedGb.toFixed(1)} GB`;
  };

  const formatBandwidth = () => {
    if (!benchmarkActive || data?.bandwidthGbps === null) return "—";
    return `${data.bandwidthGbps.toFixed(1)} GB/s`;
  };

  const formatTemperature = () => {
    if (!benchmarkActive || data?.temperatureC === null) return "—";
    return `${data.temperatureC.toFixed(1)}°C`;
  };
  ...
  const utilizationWidth =
    benchmarkActive && data?.gpuUtilization !== null
      ? Math.max(0, Math.min(100, data.gpuUtilization))
      : 0;

  const vramWidth =
    benchmarkActive &&
    data?.vramUsedGb !== null &&
    data?.vramTotalGb !== null &&
    data.vramTotalGb > 0
      ? Math.max(
          0,
          Math.min(100, (data.vramUsedGb / data.vramTotalGb) * 100)
        )
      : 0;
    ...
    <div className="page">
      <div className="page-heading">
        <div>
          ...
          <div className="subtitle">
            Live CPU vs GPU proof, measured on the same 1,000-flight recovery
            workload.
          </div>
        </div>
        ...
        <button
          ...
          disabled={running || localRunning}
        >
          {running || localRunning ? "RUNNING..." : "▷ RUN BENCHMARK"}
        </button>
      </div>
      ...
      <div className="stat-row">
        <Stat
          ...
          value={formatSpeedup()}
          description={
            benchmarkActive ? "measured CPU / GPU ratio" : "awaiting GPU benchmark"
          }
        />
        ...
        <Stat
          ...
          description={
            benchmarkActive
              ? "measured constraints / sec"
              : "awaiting GPU benchmark"
          }
        />
        ...
        <Stat
          ...
          value={formatCpuTime()}
          description={
            benchmarkActive ? "measured CPU SpMV time" : "awaiting benchmark"
          }
        />
        ...
        <Stat
          ...
          value={formatGpuTime()}
          description={
            benchmarkActive ? "measured GPU SpMV time" : "awaiting benchmark"
          }
        />
      </div>
      ...
      <div className="benchmark-grid">
        <div className="chart-card">
          ...
          <div className="chart-area">
            ...
            {/* ------------------------------------------------------------
                NO FAKE BARS.
                Bars appear only after a real benchmark result is returned.
                ------------------------------------------------------------ */}
            <div className="bars">
              {benchmarkActive && data?.throughput !== null
              ...
              {benchmarkActive && data?.cpuMs !== null
                ? Array.from({ length: 25 }).map((_, i) => (
                    <span
                      className="cpu"
                      key={`cpu-${i}`}
                      style={{
                        height: `${Math.max(4, 8 + i * 0.1)}px`,
                      }}
                    />
                  ))
                : null}
            ...
            {!benchmarkActive && (
              ...
              >
                {data?.gpuAvailable
                  ? "GPU CONNECTED · BENCHMARK NOT EXECUTED"
                  : "AWAITING GPU CONNECTION"}
              </div>
            )}
          ...
          <div className="hardware-body">
            <div className="hardware-pool">
              <span
                className="online"
                style={{
                  color: data?.gpuAvailable ? "#3caf94" : "#718096",
                }}
              >
                {data?.gpuAvailable ? "ONLINE" : "NOT CONNECTED"}
              </span>

              <div className="pool-title">
                {data?.device || "ApexCUDA GPU node"}
              </div>

              <div className="pool-subtitle">
                {data?.gpuAvailable
                  ? data?.benchmarkExecuted
                    ? "GPU benchmark node active"
                    : "GPU detected · awaiting benchmark"
                  : "Waiting for remote NVIDIA GPU"}
              </div>
            </div>
            ...
            <div className="hardware-stat">
              ...
              <div className="hardware-progress">
                <span
                  style={{
                    display: "block",
                    height: benchmarkActive && data?.bandwidthGbps !== null
                      ? "100%"
                      : "0%",
                    background: "#3caf94",
                    borderRadius: 4,
            ...
            <div className="thermal">
              ...
              <span>
                {formatTemperature()}
                {benchmarkActive && data?.temperatureC !== null
                  ? " / measured"
                  : ""}
              </span>
            </div>
      ...
      <div className="results-card">
        <div className="card-header">
          <div className="section-label">
            RESULTS / SAME 1,000-FLIGHT WORKLOAD
          </div>
          <div className="section-title">
            Large-scale solver comparison
          </div>
        </div>
        ...
        <table className="results-table">
          ...
          <tbody>
            <tr>
              <td>
                ● {data?.device || "ApexCUDA GPU"}
              </td>

              <td>{formatGpuTime()}</td>

              <td>{formatThroughput()}</td>

              <td>
                {benchmarkActive && data?.maxError !== null
                  ? data.maxError.toExponential(2)
                  : "—"}
              </td>
              ...
              <td>
                <span
                  ...
                  style={{
                    color:
                      benchmarkActive && data?.match === true
                        ? "#3caf94"
                        : "",
                  }}
                >
            ...
            <tr>
              ...
              <td>
                <span className="ready-badge">
                  {benchmarkActive ? "BASELINE" : "READY"}
                </span>
              </td>
            </tr>
          </tbody>
      ...
      {errorMessage && (
        <div
          style={{
            marginTop: 12,
            padding: "10px 14px",
            border: "1px solid #633538",
            borderRadius: 6,
            background: "rgba(85, 32, 38, 0.17)",
            color: "#9d8990",
            fontSize: 9,
            letterSpacing: 0.8,
          }}
